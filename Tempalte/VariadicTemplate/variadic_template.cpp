...
class MyClass
{
	public:
		int size ;
		int * data ;
		MyClass(int size = 0 , int *da = nullptr ) : size(size), data(new int [size])
		{
...
template <typename... T>
void function1(T&&...  ref ) // 1. variadic templte 
{
	return func(std::forward<T>(ref)...); // 2. perfect forwarding,   3.Forwarding reference  
}
